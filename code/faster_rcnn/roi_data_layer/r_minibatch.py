...
def r_get_rotate_minibatch(roidb, num_classes):
    ...
    angles = np.array(np.random.rand(num_images) * 30-15, dtype = np.int16)
    #angles = np.array(np.random.rand(num_images) * 90-45, dtype = np.int16)
    for idx in xrange(num_images):
        #background patch or plate with large angle or more than one plate
        if roidb[idx]['gt_classes'][0] ==0 or abs(roidb[idx]['boxes'][0, 4])>5 or roidb[idx]['boxes'].shape[0]>1:
            angles[idx] = 0
            roidb[idx]['rotated'] = False
        else:
            angles[idx] = angles[idx] -roidb[idx]['boxes'][0, 4]#to ensure the angle of rotated gt in[-45, 45]
...
def rotate_gt_bbox(origin_gt_roidb, angle, gt_inds):
    ...
    old_im_height = origin_gt_roidb["height"]
    old_im_width = origin_gt_roidb["width"]
    im_height = origin_gt_roidb["r_height"]
    im_width = origin_gt_roidb["r_width"]
    ...
    offset = np.array([(im_width-old_im_width)/2,(im_height-old_im_height)/2])

    pts_ctr = origin_gt_boxes[0, 0:2]+offset
    ...
    for idx in range(len_of_gt):
        ...
        rotated_gt_boxes[idx, 2] = origin_gt_boxes[idx, 2]
        rotated_gt_boxes[idx, 3] = origin_gt_boxes[idx, 3]
        rotated_gt_boxes[idx, 4] = origin_gt_boxes[idx, 4] + angle
...
def _get_rprocessed_image_blob(roidb, scale_inds, angles):
    ...
    for i in xrange(num_images):
        ...
        if roidb[i]['rotated']:
            ...
            (h, w) = im.shape[:2]
            sin = abs(np.sin(angles[i]*np.pi/180))
            cos = abs(np.cos(angles[i]*np.pi/180))
            rw = (w*cos+h*sin).astype(np.int64)
            rh = (w*sin+h*cos).astype(np.int64)
            ...
            M[0][2] += (rw-w)/2
            M[1][2] += (rh-h)/2
            im = cv2.warpAffine(im,M,(rw,rh)) 
            roidb[i]['r_width'] = rw
            roidb[i]['r_height'] = rh
            #off_set = np.array([(rw-w)/2], (rh-h)/2)
            #roidb[i]["boxes"][:, 0:2] = roidb[i]["boxes"][:, 0:2] + off_set
        im = im + cfg.PIXEL_MEANS
