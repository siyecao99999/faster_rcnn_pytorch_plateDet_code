...
def downsample(fc, layer):
    ...
    w: [512,7,7,4096] -> [512,1,4,1024]     fc6
    ...
    if fc.size(1) > 1: # weight
        if layer == 'fc6':
            fc = fc.view(1024, 512, 7, 7)[:, :, 3, 0::2].contiguous().view(1024, -1)

        elif layer == 'fc7':
            fc = fc.view(4, 1024, 1024, 1, 1)[0].view(1024,-1)
    else:
        fc = fc[:,0]
    return fc
...
def load_pretrained_npy(faster_rcnn_model, fname):
    ...
    for k, v in pairs.items():
        ...
        if 'fc6' in key:
            reduced_param = downsample(param, 'fc6')
        elif 'fc7' in key:
            reduced_param = downsample(param, 'fc7')
        frcnn_dict[key].copy_(reduced_param)
...
def load_pretrained_fcn(faster_rcnn_model, fname):
    ...
    for name, val in vgg16_dict.items():
        ...
        if name.find('bn.') >= 0:
            continue
        i, j = int(name[4]), int(name[6]) + 1
        ptype = '.weight' if name[-1] == 't' else '.bias'
        key = 'conv{}_{}'.format(i, j)+ptype
        param = params[key]
